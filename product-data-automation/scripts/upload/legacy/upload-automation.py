...
def main():
    ...
    for shopify_product in tqdm(shopify_products_to_process, desc="상품 업데이트 진행", unit="개"):
        ...
        if shopify_title_normalized in scraped_products:
            ...
            if scraped_html_content:
                # Description이 비어있거나 공백만 있는 경우에만 업데이트
                if not shopify_product.body_html or shopify_product.body_html.strip() == "":
                    try:
                    ...
                    # Description이 이미 존재하는 경우 건너뜀
                    # print(f"\n[SKIP] 상품 '{shopify_product.title}'의 Description이 이미 존재하여 건너뜁니다.") # 너무 많은 출력 방지
                    skipped_existing_desc_count += 1
                    processed_scraped_titles.add(shopify_title_normalized) 
            else:
                print(f"\n[WARNING] '{shopify_product.title}' (HTML 파일: {html_file_path})에 대한 HTML 콘텐츠를 읽을 수 없어 건너뜁니다.")
        # else:
        #     # Shopify 상품이 스크래핑된 HTML 데이터에 없는 경우
        #     # 이 경우는 스크래핑된 HTML 기준이므로 not_matched_html_count에 포함시키지 않습니다.
        #     pass
    ...
    print(f"⚠️ Description이 이미 존재하여 건너뛴 상품 수: {skipped_existing_desc_count}")
